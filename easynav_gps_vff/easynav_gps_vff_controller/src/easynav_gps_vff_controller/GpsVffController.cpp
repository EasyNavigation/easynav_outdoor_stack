...
/// \file
/// \brief Implementation of the GpsVffController class.

#include <expected>

#include "easynav_gps_vff_controller/GpsVffController.hpp"

namespace easynav
{

std::expected<void, std::string> GpsVffController::on_initialize()
{
  auto node = get_node();
  ...
  imu_subscriber_ = node->create_subscription<sensor_msgs::msg::Imu>(
    ...
    std::bind(&GpsVffController::imu_callback, this, std::placeholders::_1));
...
void GpsVffController::imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
...
geometry_msgs::msg::TwistStamped GpsVffController::get_cmd_vel()
{
  return cmd_vel_;
}

visualization_msgs::msg::MarkerArray
GpsVffController::get_debug_vff(const VFFVectors & vff_vectors, std::string frame_id)
{
...
double GpsVffController::normalize_angle(double angle)
{
...
VFFVectors GpsVffController::get_vff(
  double angle_error,
...
void GpsVffController::update_rt(const NavState & nav_state)
{
  ...
  if (!all_goals.goals.empty()) {
    ...
    double angle_error = normalize_angle(bearing - yaw_);
    ...
    auto fused =
      PerceptionsOpsView(nav_state.perceptions).fuse(cmd_vel_.header.frame_id)->filter(
...
PLUGINLIB_EXPORT_CLASS(easynav::GpsVffController, easynav::ControllerMethodBase)
